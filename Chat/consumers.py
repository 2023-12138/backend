...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if tid == "":   #私聊
            ...
            if not toUserSocket:        #成员在线
                await toUserSocket.send(text_data=json.dumps({"message": message}))
            await self.send(text_data=json.dumps({"message": message}))  # 在自己窗口展示
        if to_uid == "":
            ...
            for user in userlist:
                ...
                if not toUserSocket:            #成员在线
                    await toUserSocket.send(text_data=json.dumps({"message": message}))
        new_record = Record(cid=cid, time=datetime.datetime, content=message, sender=from_uid)
        try:
            new_record.save()
        except:
