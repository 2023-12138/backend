    ...
    async def receive(self, text_data):
        ...
        if msgType == "chat" or msgType == "chat_pic" or msgType == "chat_file":  # 是聊天消息
            ...
            if tid == "":  # 私聊
                ...
                data = {"message": message, "senderId": self.uid, "senderName": self.uname, "receiverId": to_uid,
                        "teamId": tid, "time": nowTime,
                        "rid": new_record.rid}
            ...
            if to_uid == "":  # 群聊
                ...
                if aite != None and len(aite) > 0:
                    for i in aite:
                        if i == -1:  # @所有人
                            async for user in userlist:
                                ...
                                if toUserSocket != None:  # 成员在线
                                    data = {"message": message, "senderId": self.uid, "senderName": self.uname,
                                            "receiverId": "", "teamId": tid,
                                            "time": nowTime,
                                            "rid": new_record.rid}
                                    await toUserSocket.send(text_data=json.dumps(
                                        {
                    ...
                    async for user in userlist:
                        data = {"message": message, "senderId": self.uid, "senderName": self.uname, "receiverId": "",
                                "teamId": tid,
                                "time": nowTime,
                                "rid": new_record.rid}
                        toUserSocket = userSocketDict.get(user.uid)
